...
const char* _libinject_log_tag = "LIBINJECT";
...
static long trace(const char* debug, int request, void *addr = NULL, size_t data = 0) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (ret == -1 && (errno == EBUSY || errno == EFAULT || errno == ESRCH)) {
            ...
            LOGD("ptrace [%s] error [%s] on request [%s]", debug, e, r);
        }
        if (ret == -1 && (errno == ESRCH)) {
			LOGD("ptrace remote_stop/retry");
...
int libinject_inject(pid_t pid, char* library) {
    ...
    if ( trace( "attach", PTRACE_ATTACH ) != -1) {
		...
		LOGD( "calloc:%p free:%p dlopen:%p dlerror:%p", (void*)_calloc, (void*)_free, (void*)_dlopen, (void*)_dlerror );
		...
		LOGD( "Failed to attach to process %d", _pid);
    }
